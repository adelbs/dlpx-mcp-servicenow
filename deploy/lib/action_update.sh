...
action_update() {
    ...
    log_info "Re-uploading app/, pyproject.toml, uv.lock and README.md to the server..."
    ...
    # uv.lock pins the exact resolved versions tested against this codebase —
    # `uv sync --frozen` below installs exactly that, instead of letting the
    # server re-resolve the dependency graph fresh (see docs/architecture.md).
    upload_file "$REPO_ROOT/uv.lock" "/tmp/dlpx-svcnow-orch-update/uv.lock"
    ...
    remote_sudo "rm -rf /opt/dlpx-servicenow-orchestrator/app /opt/dlpx-servicenow-orchestrator/pyproject.toml /opt/dlpx-servicenow-orchestrator/uv.lock /opt/dlpx-servicenow-orchestrator/README.md && \
        ...
        cp /tmp/dlpx-svcnow-orch-update/uv.lock /opt/dlpx-servicenow-orchestrator/uv.lock && \
        ...
        sudo -u svcnow-orch -H bash -c 'cd /opt/dlpx-servicenow-orchestrator && /usr/local/bin/uv sync --frozen'"
